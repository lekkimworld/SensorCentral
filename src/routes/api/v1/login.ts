...
import { getAuthenticationUrl, getConfiguredProviders, AuthenticationUrlPayload } from "../../../oidc-authentication-utils";
...
/**
 * Returns the list of configured OIDC providers.
 */
router.get("/providers", (_req, res) => {
    res.send(getConfiguredProviders());
});
...
router.get("/:provider", async (req, res, next) => {
    ...
    try {
        ...
        // save nonce and code verifier, save session
        ...
        return next(new HttpException(500, `Unable to initiate login for provider <${req.params.provider}>`, err));
    }
})
...
router.get("/jwt{/:houseId}", ensureAuthenticated, async (req, res, next) => {
    ...
    // get houseid for jwt
    let houseId = req.params.houseId as string | undefined;
